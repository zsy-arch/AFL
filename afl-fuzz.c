...
int main(int argc, char** argv) {
  ...
  // 通过AFL_PRELOAD设置目标程序的LD_PRELOAD和DYLD_INSERT_LIBRARIES环境变量
  ...
  // 检查/proc/sys/kernel/core_pattern是否误配置，避免引入延时
  check_crash_handling();
  check_cpu_governor();

  // 设置post_handler，用于自定义用例
  setup_post();
  // 设置共享内存
  setup_shm();
  init_count_class16(); // 用于性能优化
